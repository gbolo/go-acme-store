...
func MustInitViperAndLogger(appName, cfgFile string) {
	...
	logConfig := initLoggerFallbackToDefault()
	...
	// set the app name if needed. then log the metadata that should be set by Makefile
	if meta.Name == "unknown" || meta.Name == "" {
		meta.Name = appName
	}
	if logConfig.Encoding == "json" {
		log.Infow("initializing app", "meta_data", meta.GetAppMetadata())
	} else {
		log.Infof("initializing app: %v", meta.GetAppMetadata().ToString())
	}
}
